...
export default function UsagePage() {
    ...
    const fetchImages = async () => {
        ...
        const [createdImgs, editedImgs] = await Promise.all([
            supabase.from("images_created").select().eq("user_id", user?.id)
                .gte("created_at", thirtyDaysAgoISOString),
            supabase.from("images_edited").select().eq("user_id", user?.id)
                .gte("created_at", thirtyDaysAgoISOString),
        ])
    ...
    return (
        <div className="bg-black min-h-screen w-full items-center justify-start text-black flex flex-col">
            <Header />
            <div className="min-h-[50vh] items-center justify-start
